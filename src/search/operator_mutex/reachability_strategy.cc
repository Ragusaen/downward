...
* to be a DAG.
*/
bool GoalReachability::reachability(const CondensedTransitionSystem &cts, vector<bool> &reach, int state, const std::unordered_set<int> &unreachable_states) {
...
*/
void NoGoalReachability::reachability(const CondensedTransitionSystem &cts, std::vector<bool> &reach, int state, const std::unordered_set<int> &unreachable_states) {
    // Check whether this state has been visited before, states can always reach themselves
    if (!REACH_XY(state, state)) {
        // Flag that the current state can reach itself
        REACH_XY(state, state) = true;

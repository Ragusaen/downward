...
CONFIGS = [
    ...
    for index, (h_nick, op_mutex) in enumerate(
            ...
            # 	("s_fh()", "shrink_fh()")]
            # ]
            [(f"{o_nick}", f"op_mutex(use_previous_ops={o}, reachability_strategy=goal)") 
            for (o_nick, o) in [
            	("NoPO", "NoPO"), 
            	("NaSUTPO", "NaSUTPO"),
            	("NeLUTPO", "NeLUTPO")]
